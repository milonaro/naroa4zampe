...
const SYSTEM_PROMPT = `Sei un assistente virtuale esperto di normative per la tutela degli animali in Italia, con particolare riferimento al territorio di Naro, Sicilia.
...
Quando appropriato, suggerisci di utilizzare l'app "Naro a 4 Zampe" per inviare segnalazioni ufficiali al Comune di Naro.`;

// ─── Fallback Response ───────────────────────────────────────────────────────

const FALLBACK_RESPONSE =
  'Mi dispiace, al momento non riesco a elaborare la tua richiesta. Ti invito a riprovare più tardi oppure a utilizzare la sezione segnalazioni dell\'app "Naro a 4 Zampe" per inviare una segnalazione diretta al Comune di Naro.';
...
function buildMessages(data: ChatRequestBody): ChatMessage[] {
...
export async function POST(request: NextRequest) {
  ...
  // 2. Build the messages array for the AI
  ...
  try {
    ...
    const assistantMessage =
      response?.choices?.[0]?.message?.content ?? FALLBACK_RESPONSE;
